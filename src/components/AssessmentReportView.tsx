/**
 * Shared, presentational render of a confirmed workshop assessment ("مقايسة إصلاح").
 * Used by both the workshop report page (Report.tsx) and the broker portal's
 * public report page (BrokerAssessment.tsx) so both show identical UI + data.
 * Pure/read-only: no data fetching, no status actions.
 */

export type EstimateStatus =
  | 'draft' | 'confirmed' | 'approved_by_insurance' | 'rejected_by_insurance'
  | 'counter_offer' | 'workshop_revised' | 'workshop_accepted' | 'settled'

export interface LaborGroup { labor_name_ar: string; total: number; parts: string[] }
export interface PartPrice { part_name_ar: string; price: number }
...
export interface ReportChainItem {
  estimate_id: string
...
export interface AssessmentReport {
  ...
  vin_number?: string; customer_name?: string; customer_mobile?: string
...
export const STATUS_CONFIG: Record<EstimateStatus, { label: string; color: string; bg: string; border: string }> = {
...
/**
 * @param chainHrefBase where chain items link (workshop: '/report', broker: '/assessment')
 * @param showPrint      render the print button (both do by default)
 */
export default function AssessmentReportView({
  report, chain = [], currentEstimateId, chainHrefBase = '/report', showPrint = true, footer,
}: {
  report: AssessmentReport
  chain?: ReportChainItem[]
  currentEstimateId: string
  chainHrefBase?: string
  showPrint?: boolean
  /** Caller-specific controls rendered inside the card, after the print button (e.g. workshop Share/Back). */
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f3f4f6', padding: '0.5rem 0.25rem', direction: 'rtl' }}>
      <style>{`@media print { .gf-no-print { display: none !important; } body { background: #fff !important; } @page { margin: 12mm; } }`}</style>
        ...
        <div style={{ backgroundColor: 'white', borderRadius: '0.5rem', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)', overflow: 'hidden', padding: '1rem 0.75rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '0.75rem', marginBottom: '1.25rem', fontSize: '0.9rem' }}>
            ...
            <div style={{ textAlign: 'right' }}>
              ...
              {infoField('سنه الصنع', report.vehicle_year ? String(report.vehicle_year) : '-')}
            </div>
          </div>
            ...
            <div style={{ marginBottom: '2rem', padding: '1rem 1.25rem', backgroundColor: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '0.6rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {chain.map((item, idx) => {
                  ...
                  const isThis = item.estimate_id === currentEstimateId
                  const isSupplement = !!item.parent_estimate_id
                  return (
                    <a
                      key={item.estimate_id}
                      href={isThis ? undefined : `${chainHrefBase}/${item.estimate_id}`}
                      style={{
            ...
            <>
              ...
              {(pd.repair_groups.length > 0 || pd.replace_groups.length > 0) && (() => {
                ...
                ;[...pd.repair_groups, ...pd.replace_groups].forEach(g => {
                  ...
                  if (existing) { existing.parts = [...existing.parts, ...g.parts]; existing.total += g.total }
                  else merged.set(g.labor_name_ar, { parts: [...g.parts], total: g.total })
                })
          ...
          {showPrint && (
            <button onClick={() => window.print()} className="gf-no-print"
              style={{ width: '100%', padding: '1rem', backgroundColor: '#1e3a8a', color: 'white', border: 'none', borderRadius: '0.375rem', fontWeight: 'bold', cursor: 'pointer', marginBottom: '1rem', fontSize: '0.95rem' }}>
              🖨️ طباعة المقايسة
            </button>
          )}
