...
router.get('/workshops', authenticate, requireSuperAdmin, async (req, res, next) => {
  try {
    const { data, error } = await supabase
      .from('workshops')
      .select('workshop_id, workshop_name, display_name, city, phone, is_active, is_visible_to_consumers, is_super_admin, stars, review_text, is_new, badges, sort_order, created_at, logo_url, accepts_insurance, working_days, google_place_id')
...
router.patch('/workshops/:id', authenticate, requireSuperAdmin, async (req, res, next) => {
  try {
    const { id } = req.params;
    const allowed = ['display_name', 'is_visible_to_consumers', 'stars', 'review_text', 'is_new', 'badges', 'sort_order', 'is_active', 'logo_url', 'accepts_insurance', 'working_days', 'google_place_id'];
    const updates = {};
    ...
    const { data, error } = await supabase
      ...
      .update(updates)
      .eq('workshop_id', id)
      .select('workshop_id, workshop_name, display_name, city, is_visible_to_consumers, stars, review_text, is_new, badges, sort_order, is_active, logo_url, accepts_insurance, working_days, google_place_id')
      .single();
